...
bool RichGraph::loadFromDatabase(const std::string& map_name, std::shared_ptr<drogon::orm::DbClient> db_client) {
    ...
    try {
        ...
        drogon::orm::Mapper<drogon_model::amr_01::amr_ros2::Straightlink> linkMapper(db_client);
        auto linkCriteria = drogon::orm::Criteria(drogon_model::amr_01::amr_ros2::Straightlink::Cols::_map_id, drogon::orm::CompareOperator::EQ, current_map_id_);
        auto db_links = linkMapper.findBy(linkCriteria);
        
        for (const auto& db_link : db_links) {
            LinkInfo link = vrobot_route_follow::utils::DatabaseConverter::convertLink(db_link);
...
std::optional<int32_t> RichGraph::getLinkBetweenNodes(int32_t start_node, int32_t end_node) const {
...
std::vector<int32_t> RichGraph::findClosestNodes(const Eigen::Vector3d& pose, size_t max_nodes) const {
...
std::vector<int32_t> RichGraph::findClosestLinks(const Eigen::Vector3d& pose, size_t max_links) const {
...
double RichGraph::getDistanceToNode(const Eigen::Vector3d& pose, int32_t node_id) const {
...
double RichGraph::getDistanceToLink(const Eigen::Vector3d& pose, int32_t link_id) const {
    const auto& link = links_.at(link_id);
...
RichPathResult RichGraph::planPath(const Eigen::Vector3d& start_pose, int32_t target_node_id, const PlanningConfig& config) const {
...
RichPathResult RichGraph::planPath(int32_t start_node_id, int32_t target_node_id, const PlanningConfig& config) const {
...
RichPathResult RichGraph::planPath(const Eigen::Vector3d& start_pose, const Eigen::Vector3d& target_pose, const PlanningConfig& config) const {
...
double RichGraph::calculateHeuristic(const NodeInfo& current, const NodeInfo& goal, PlanningConfig::Algorithm algorithm) const {
    switch (algorithm) {
        case PlanningConfig::Algorithm::A_STAR: {
        ...
        default:
            return 0.0;  // No heuristic for Dijkstra
    }
}

std::vector<int32_t> RichGraph::reconstructPath(const std::unordered_map<int32_t, int32_t>& parent, int32_t goal_node) const {
...
RichPathResult RichGraph::planDijkstra(int32_t start_node, int32_t target_node, const PlanningConfig& config) const {
    RichPathResult result;
    ...
    while (!pq.empty()) {
        if (config.max_nodes_explored && nodes_explored >= *config.max_nodes_explored) {
            break;
        }
        ...
        if (current_node == target_node) {
            ...
            for (int32_t node_id : node_path) {
                ...
                result.poseSequence.push_back(Eigen::Vector3d(node.x, node.y, node.theta));
            }
        ...
        for (int32_t link_id : getNodeNeighborLinks(current_node)) {
            ...
            if (config.custom_scorer) {
                ...
                edge_weight = config.custom_scorer(link, current_node_info, neighbor_node_info);
            }
            ...
            if (config.max_search_distance && new_distance > *config.max_search_distance) {
                continue;
            }
...
RichPathResult RichGraph::planAStar(int32_t start_node, int32_t target_node, const PlanningConfig& config) const {
    RichPathResult result;
    ...
    f_score[start_node] = calculateHeuristic(getNode(start_node), target_node_info, PlanningConfig::Algorithm::A_STAR);
    ...
    while (!pq.empty()) {
        ...
        for (int32_t link_id : getNodeNeighborLinks(current_node)) {
            ...
            if (g_score.find(neighbor) == g_score.end() || tentative_g < g_score[neighbor]) {
                ...
                double h_score = calculateHeuristic(getNode(neighbor), target_node_info, PlanningConfig::Algorithm::A_STAR);
...
RichPathResult RichGraph::planDirectPath(const Eigen::Vector3d& start_pose, int32_t target_node, const PlanningConfig& config) const {
    RichPathResult result;
    result.algorithmUsed = "DirectPath";
    
    const auto& target_node_info = getNode(target_node);
    Eigen::Vector3d target_pose(target_node_info.x, target_node_info.y, target_node_info.theta);
...
RichPathResult RichGraph::planLinkBased(const Eigen::Vector3d& start_pose, int32_t target_node, const PlanningConfig& config) const {
    RichPathResult result;
    ...
    for (int32_t link_id : closest_links) {
        ...
        if (path_from_link.success) {
            ...
            if (total_distance < best_distance) {
                ...
                best_path.poseSequence.insert(best_path.poseSequence.begin(), start_pose);
...
double RichGraph::calculatePathLength(const std::vector<int32_t>& node_sequence) const {
...
RichGraph::GraphStatistics RichGraph::getStatistics() const {
    ...
    if (!nodes_.empty()) {
        ...
        for (const auto& [node_id, node] : nodes_) {
            stats.bounding_box_min.first = std::min(stats.bounding_box_min.first, static_cast<double>(node.x));
            stats.bounding_box_min.second = std::min(stats.bounding_box_min.second, static_cast<double>(node.y));
            stats.bounding_box_max.first = std::max(stats.bounding_box_max.first, static_cast<double>(node.x));
            stats.bounding_box_max.second = std::max(stats.bounding_box_max.second, static_cast<double>(node.y));
        }
    }
    ...
    if (!nodes_.empty()) {
        ...
        stats.average_node_degree = static_cast<double>(total_degree) / nodes_.size();
    }
    
    // Graph density = 2 * |E| / (|V| * (|V| - 1))
    if (nodes_.size() > 1) {
        stats.graph_density = (2.0 * links_.size()) / (nodes_.size() * (nodes_.size() - 1));
    }
    
    // Connected components (simplified - just check if graph is connected)
    stats.connected_components = 1;  // TODO: Implement proper connected components calculation
...
RichGraph::ValidationResult RichGraph::validateGraph() const {
    ...
    for (const auto& [node_id, node] : nodes_) {
        if (getNodeNeighborLinks(node_id).empty() && getNodeIncomingLinks(node_id).empty()) {
            result.warnings.push_back("Orphaned node found: " + std::to_string(node_id));
        }
    }
    ...
    for (const auto& [link_id, link] : links_) {
        if (!hasNode(link.id_start)) {
            result.errors.push_back("Link " + std::to_string(link_id) + " has invalid start node: " + std::to_string(link.id_start));
            result.is_valid = false;
        }
        if (!hasNode(link.id_end)) {
            result.errors.push_back("Link " + std::to_string(link_id) + " has invalid end node: " + std::to_string(link.id_end));
            result.is_valid = false;
        }
    }
