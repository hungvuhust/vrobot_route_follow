...
RichDatabaseLoader::RichDatabaseLoader(const std::string &connection_info) {
...
LoadStatistics RichDatabaseLoader::loadMap(const std::string        &map_name,
  ...
  if (config.enable_cache) {
    ...
    if (cached_data && isCacheValid(*cached_data, config)) {
      ...
      current_curved_links_   = &current_cache.curved_links;
...
const std::unordered_map<int32_t, NodeInfo> &
RichDatabaseLoader::getNodes() const {
  if (!current_nodes_) {
...
const std::unordered_map<int32_t, LinkInfo> &
RichDatabaseLoader::getLinks() const {
  if (!current_links_) {
    throw std::runtime_error("No map data loaded. Call loadMap() first.");
  }
...
const std::unordered_map<int32_t, CurveLinkInfo> &
RichDatabaseLoader::getCurvedLinks() const {
...
RichDatabaseLoader::getAdjacencyList() const {
...
RichDatabaseLoader::getCacheStatistics() const {
  std::unordered_map<std::string, double> stats;

  size_t total_requests = cache_hits_ + cache_misses_;
  stats["cache_hit_rate"] =
      total_requests > 0 ? static_cast<double>(cache_hits_) / total_requests
                         : 0.0;
...
LoadStatistics RichDatabaseLoader::loadMapInBounds(
    const std::string &map_name, const std::pair<double, double> &min_coords,
...
LoadStatistics RichDatabaseLoader::loadMapByNodeTypes(
    const std::string &map_name, const std::vector<std::string> &node_types) {
  DatabaseLoadConfig config = default_config_;
...
std::vector<std::string> RichDatabaseLoader::validateDataIntegrity() const {
  ...
  for (const auto &[link_id, link] : *current_links_) {
    if (current_nodes_->find(link.id_start) == current_nodes_->end()) {
      issues.push_back("Link " + std::to_string(link_id) +
                       " references non-existent start node " +
                       std::to_string(link.id_start));
    }
    if (current_nodes_->find(link.id_end) == current_nodes_->end()) {
      issues.push_back("Link " + std::to_string(link_id) +
                       " references non-existent end node " +
                       std::to_string(link.id_end));
    }
  }
  ...
  for (const auto &[node_id, link_ids] : *current_adjacency_list_) {
    if (current_nodes_->find(node_id) == current_nodes_->end()) {
      issues.push_back("Adjacency list contains non-existent node " +
    ...
    for (const auto &link_id : link_ids) {
      if (current_links_->find(link_id) == current_links_->end()) {
        issues.push_back("Adjacency list references non-existent link " +
...
LoadStatistics
RichDatabaseLoader::loadFromDatabase(const std::string        &map_name,
                                     const DatabaseLoadConfig &config) {
  auto           start_time = std::chrono::steady_clock::now();
  ...
  try {
    ...
    auto curved_links_vec = loadCurvedLinks(stats.map_id, config);
    ...
    throw std::runtime_error("Failed to load map '" + map_name +
                             "': " + e.what());
  }
...
RichDatabaseLoader::getMapId(const std::string &map_name) {
  try {
    ...
    std::cerr << "Error getting map ID for '" << map_name << "': " << e.what()
              << std::endl;
    return std::nullopt;
  }
}
...
std::vector<NodeInfo>
RichDatabaseLoader::loadNodes(int32_t                   map_id,
                              const DatabaseLoadConfig &config) {
  std::vector<NodeInfo> nodes;

  try {
    drogon::orm::Mapper<drogon_model::amr_01::amr_ros2::Node> mapper(
        db_client_);
    auto criteria = drogon::orm::Criteria(
        drogon_model::amr_01::amr_ros2::Node::Cols::_map_id,
        drogon::orm::CompareOperator::EQ, map_id);
...
std::vector<LinkInfo>
RichDatabaseLoader::loadLinks(int32_t                   map_id,
                              const DatabaseLoadConfig &config) {
  std::vector<LinkInfo> links;

  try {
    drogon::orm::Mapper<drogon_model::amr_01::amr_ros2::Straightlink> mapper(
        db_client_);
    auto criteria = drogon::orm::Criteria(
...
std::vector<CurveLinkInfo>
RichDatabaseLoader::loadCurvedLinks(int32_t                   map_id,
                                    const DatabaseLoadConfig &config) {
  std::vector<CurveLinkInfo> curved_links;

  try {
    drogon::orm::Mapper<drogon_model::amr_01::amr_ros2::Curvelink> mapper(
        db_client_);
    auto criteria = drogon::orm::Criteria(
        drogon_model::amr_01::amr_ros2::Curvelink::Cols::_map_id,
        drogon::orm::CompareOperator::EQ, map_id);

    auto db_curved_links = mapper.findBy(criteria);
    curved_links.reserve(db_curved_links.size());

    for (const auto &db_curved_link : db_curved_links) {
      curved_links.push_back(
          utils::DatabaseConverter::convertCurveLink(db_curved_link));
    }

  } catch (const std::exception &e) {
    throw std::runtime_error("Failed to load curved links: " +
                             std::string(e.what()));
  }
...
void RichDatabaseLoader::updateCache(const std::string &map_name,
                                     CachedMapData    &&data) {
...
std::optional<CachedMapData>
RichDatabaseLoader::getCachedData(const std::string &map_name) const {
...
bool RichDatabaseLoader::isCacheValid(const CachedMapData      &data,
                                      const DatabaseLoadConfig &config) const {
  if (!config.enable_cache) {
    return false;
  }
  ...
  auto age =
      std::chrono::duration_cast<std::chrono::minutes>(now - data.loaded_at);

  return age < config.cache_ttl;
}

void RichDatabaseLoader::buildAdjacencyList(
    const std::vector<NodeInfo> &nodes, const std::vector<LinkInfo> &links,
...
void RichDatabaseLoader::applyFilters(std::vector<NodeInfo>    &nodes,
  ...
  if (config.node_types_filter) {
    ...
    nodes.erase(std::remove_if(nodes.begin(), nodes.end(),
                               [&allowed_types](const NodeInfo &node) {
                                 return std::find(allowed_types.begin(),
                                                  allowed_types.end(),
                                                  node.type) ==
                                        allowed_types.end();
                               }),
  ...
  links.erase(std::remove_if(links.begin(), links.end(),
                             [&valid_node_ids](const LinkInfo &link) {
                               return valid_node_ids.find(link.id_start) ==
                                          valid_node_ids.end() ||
                                      valid_node_ids.find(link.id_end) ==
                                          valid_node_ids.end();
                             }),
              links.end());
}
