...
public:
  ...
  explicit SimpleActionServer(
      ...
      : node_base_interface_(node_base_interface),
        ...
        server_timeout_(server_timeout), spin_thread_(spin_thread) {
    ...
    action_server_ = rclcpp_action::create_server<ActionT>(
        ...
        std::bind(&SimpleActionServer::handle_accepted, this, _1), options,
        callback_group_);
    if (spin_thread_) {
      executor_ = std::make_shared<rclcpp::executors::SingleThreadedExecutor>();
      executor_->add_callback_group(callback_group_, node_base_interface_);
      executor_thread_ =
          std::make_unique<vrobot_route_follow::NodeThread>(executor_);
    }
  }
...
protected:
  ...
  std::unique_ptr<vrobot_route_follow::NodeThread>     executor_thread_;
