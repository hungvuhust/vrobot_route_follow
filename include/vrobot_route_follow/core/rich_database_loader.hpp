...
public:
  ...
  explicit RichDatabaseLoader(const std::string &connection_info);
   ...
   */
  LoadStatistics
  loadMap(const std::string        &map_name,
   ...
   */
  LoadStatistics
  reloadMap(const std::string        &map_name,
            const DatabaseLoadConfig &config = DatabaseLoadConfig{});
...
private:
  ...
  std::vector<NodeInfo>      loadNodes(int32_t                   map_id,
                                       const DatabaseLoadConfig &config);
  std::vector<LinkInfo>      loadLinks(int32_t                   map_id,
                                       const DatabaseLoadConfig &config);
  std::vector<CurveLinkInfo> loadCurvedLinks(int32_t                   map_id,
                                             const DatabaseLoadConfig &config);
  ...
  bool                         isCacheValid(const CachedMapData      &data,
  ...
  void buildAdjacencyList(
      ...
      std::unordered_map<int32_t, std::vector<int32_t>> &adjacency_list);
  void applyFilters(std::vector<NodeInfo> &nodes, std::vector<LinkInfo> &links,
                    const DatabaseLoadConfig &config);
  ...
  const std::unordered_map<int32_t, CurveLinkInfo> *current_curved_links_ =
      nullptr;
  const std::unordered_map<int32_t, std::vector<int32_t>>
